...
const UnifiedDashboard = () => {
  ...
  // End Auction (destructive) — reuses the existing /finish endpoint + confirm.
  const handleEndAuction = async () => {
    const ok = await confirm(
      'This permanently ends the auction. This cannot be undone.',
      'End Auction?'
    );
    if (!ok) return;
    try {
      const res = await axios.post(`${API_BASE_URL}/api/auction/finish`);
      showSuccess(res.data?.message || 'Auction ended', 'Auction Complete');
    } catch (error) {
      showError(error.response?.data?.error || 'Could not end the auction');
    }
  };
          ...
          >
            <div className="p-6">
              <div className="flex items-center justify-center mb-4">
                <div className={`flex items-center justify-center h-12 w-12 rounded-full ${undoConfirmAction?.type === 'sale' ? 'bg-rose-500/15 text-rose-300' : 'bg-amber-500/15 text-amber-300'}`}>
              ...
              <div className="flex flex-col sm:flex-row gap-3">
                ...
                <button
                  ...
                  className={`flex-1 font-semibold py-2.5 px-4 rounded-full text-white transition-colors disabled:opacity-50 ${
