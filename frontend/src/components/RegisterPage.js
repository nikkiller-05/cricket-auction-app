...
const RegisterPage = () => {
  ...
  const [form, setForm] = useState({
    ...
    matches: '', runs: '', wickets: '', battingHand: '', bowlingStyle: '', paymentTxnId: '', website: '',
  });
          ...
          <div className="grid grid-cols-2 gap-3">
            <Field label="Batting hand">
              <select className={inputCls} value={form.battingHand} onChange={set('battingHand')}>
                {BATTING_HANDS.map((o) => <option key={o} value={o}>{o || 'Select…'}</option>)}
              </select>
            </Field>
            <Field label="Bowling style">
              <select className={inputCls} value={form.bowlingStyle} onChange={set('bowlingStyle')}>
                {BOWLING_STYLES.map((o) => <option key={o} value={o}>{o || 'Select…'}</option>)}
              </select>
            </Field>
          </div>
